    ...
    <motion.div
      ...
      >
        <p onMouseEnter={()=>{setIsHovered(true)}} onMouseLeave={()=>{setIsHovered(false)}}
          className=' text-3xl w-[75%] text-center font-medium mb-20'
          >
          ...
          <div className="flex flex-col justify-center items-center">
            {["journal through your", "life journey with", "FORCE"].map(
              (value, index) => (
                <div
                  key={index}
                  className="text-orange-400 text-7xl font-medium capitalize z-10 cursor-pointer tracking-tighter mt-4"
                >
                  {value}
                </div>
              )
            ...
            <button className="px-6 py-3 rounded-full bg-orange-400 text-white text-xl mt-8 text-nowrap flex items-center justify-center gap-5 hover:bg-white hover:text-orange-400 hover:scale-125 transition-transform duration-1000">
              Start today
            </button>
            <p className="text-white font-bold mt-3">Find- The QR-Code to download</p>
          </div>
        </div>
        <Phone/>
    </>
