...
export default function InfiniteMenu({ items = [] }) {
  ...
  useEffect(() => {
    ...
    // This function uses the ref value instead of the state
    const handleActiveItem = (index) => {
      const actualItems = itemsRef.current.length ? itemsRef.current : defaultItems;
      const itemIndex = index % actualItems.length;
      setActiveItem(actualItems[itemIndex]);
    };

    if (canvas) {
      gridMenuRef.current = new InfiniteGridMenu(
        canvas,
    ...
    const handleResize = () => {
      if (gridMenuRef.current) {
        gridMenuRef.current.resize();
      }
    };
